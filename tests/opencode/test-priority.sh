...
# Verifies that skills are resolved with correct priority: project > personal > forge
...
# 1. Create in forge location (lowest priority)
mkdir -p "$HOME/.config/opencode/forge/skills/priority-test"
cat > "$HOME/.config/opencode/forge/skills/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
...
if [ -f "$HOME/.config/opencode/forge/skills/priority-test/SKILL.md" ]; then
...
# Test 2: Test that personal overrides forge
echo ""
echo "Test 2: Testing personal > forge priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides forge)"
    ...
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|forge" | head -10
fi

# Test 3: Test that project overrides both personal and forge
echo ""
echo "Test 3: Testing project > personal > forge priority..."
...
# Test 4: Test explicit forge: prefix bypasses priority
echo ""
echo "Test 4: Testing forge: prefix forces forge version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load forge:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_SUPERPOWERS_VERSION"; then
    echo "  [PASS] forge: prefix correctly forces forge version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] forge: prefix did not force forge version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
