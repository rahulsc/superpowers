...
8. **Design review loop** — dispatch reviewer subagent with precisely crafted review context (never your session history), iterate until approved (max 5 rounds)
9. **User reviews written spec** — ask user to review the spec file before proceeding
10. **Create worktree** — create project worktree via using-git-worktrees, record path in state.yml
11. **Compose team?** — apply the decision framework below; invoke superpowers:composing-teams only if criteria met; transition to implementation via superpowers:writing-plans
